...
extern struct wake_lock jrd_vbus_wake_lock;
...
struct usb_info *for_usb_wakelok =NULL;
	...
	for (;;) {
		...
		case USB_STATE_ONLINE:
			...
			if (flags & USB_FLAG_VBUS_OFFLINE) {
				...
				msm72k_pm_qos_update(0);
				wake_unlock(&ui->wlock);
                                wake_unlock(&jrd_vbus_wake_lock);//jrd unlock wake lock
				break;
			}
...
extern void jrd_msm_otg_start_peripheral_equ(void);

void jrd_unlock_usb_wakelock(void)
{
	printk("@@@@@  ulock wakelock entry\n");
	if (for_usb_wakelok)
	{
	      printk("@@@@@  ulock wakelock\n");
              jrd_msm_otg_start_peripheral_equ(); 
	}
}
